-- ServerScriptService/Abilities/SurvivorAbilities/DummyDive.lua (VERSI�N FINAL CON ORDEN AL CLIENTE)
...
DummyDive.RequiredEvents = { { Name = "AbilityUsed" }, { Name = "ApplyState" } } -- <-- A�ADIMOS EL NUEVO EVENTO
...
function DummyDive.Execute(player)
	...
	-- 1. Disparar evento para la animaci�n de INICIO
	...
	task.spawn(function()
		-- ... la l�gica de la hitbox se queda igual ...
	end)
	...
	task.delay(ABILITY_CONFIG.DiveDuration, function()
		if player and Events.ApplyState then
			-- Orden: "Jugador, apl�cate el estado 'Stunned' durante X segundos"
			Events.ApplyState:FireClient(player, "Stunned", ABILITY_CONFIG.SelfStunDuration)

			-- Tambi�n disparamos la animaci�n de Stun para todos los dem�s
			if Events.AbilityUsed then
				Events.AbilityUsed:FireAllClients(character, DummyDive.Name, "Stun", role, charName)
			end
		end
	end)
	-- 5. [[ CORRECCIÓN ]] Asegurar que el stun termine exactamente cuando debe
