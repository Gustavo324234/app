-- ServerScriptService/Modules/SetupManager.lua (VERSI�N CORREGIDA Y COMPLETA)
-- Este m�dulo se encarga de crear todas las instancias compartidas al inicio del juego
...
function SetupManager.Initialize()
	print("[SetupManager] Iniciando la configuraci�n de instancias compartidas...")
	...
	local eventNames = {
		...
		"UpdateTimer",        -- Servidor -> Cliente. Env�a el tiempo restante de la ronda para actualizar la UI.
		"PlayerDied",         -- Servidor -> Todos los Clientes. Notifica que un jugador ha muerto, �til para contadores, UI, etc.
		...
		"UpdateAbilityUI",    -- Servidor -> Cliente. Env�a datos para actualizar la UI de habilidades de un jugador (cooldowns, iconos).
		"PlayerAttack",       -- Cliente -> Servidor. El Asesino solicita realizar un ataque b�sico.
		"ShowDebuffUI",       -- Servidor -> Cliente. Indica a un jugador que muestre un icono de efecto negativo (ralentizado, quemado, etc.).
		"ApplyState",         -- Servidor -> Cliente. Ordena al cliente que se aplique a s� mismo un estado (ej. "Stunned") para resolver problemas de Network Ownership.
		"ObscureVision",      -- Servidor -> Cliente. Un efecto de habilidad espec�fico que le dice al cliente que su visi�n debe ser oscurecida.
